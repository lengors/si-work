...
def depthFirstSearch(problem):
    ...
    stack.push((current_state, list()))
    states.add(current_state)
    while not stack.isEmpty():
        current_state, *remain = stack.pop()
        ...
        for state, direction, stuff_idk in successors:
            if state not in states:
                stack.push((state, remain[-1] + [direction]))
                states.add(state)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    states = set()
    queue = util.PriorityQueueWithFunction(lambda item : problem.getCostOfActions(item[1]) + heuristic(item[0], problem))
    current_state = problem.getStartState()
    queue.push((current_state, list()))
    states.add(current_state)
    while not queue.isEmpty():
